defmodule ScaleGenerator do
  ...
  def step(scale, tonic, step) do
    tonic_index = Enum.find_index(scale, &(&1 == parse_note tonic))

    case step do
      "m" -> Enum.at(scale, tonic_index+1)
      "M" -> Enum.at(scale, tonic_index+2)
      "A" -> Enum.at(scale, tonic_index+3)
    end
  end
  ...
  def chromatic_scale(tonic \\ "C", scale \\ Stream.cycle(~w(C C# D D# E F F# G G# A A# B))) do
    tonic_index =
      scale
      |> Enum.take(12)
      |> Enum.find_index(&(&1 == parse_note tonic))

    scale
    |> Enum.take(tonic_index + 13)
    |> Enum.slice(tonic_index, tonic_index + 13)
  end
  ...
  def flat_chromatic_scale(tonic \\ "C") do
    tonic
    |> chromatic_scale(Stream.cycle(~w(C Db D Eb E F Gb G Ab A Bb B)))
  end
  ...
  def find_chromatic_scale(tonic) do
    cond do
      tonic in ~w(F Bb Eb Ab Db Gb d g c f bb eb) ->
  ...
  def scale(tonic, pattern) do
    ...
    pattern
    |> String.graphemes()
    |> Enum.reduce([parse_note tonic], fn interval, [t|_] = acc ->
      [step(scale, t, interval)|acc]
    end)
    |> Enum.reverse()
  end

  @spec parse_note(note :: String.t()) :: String.t()
  defp parse_note(note) do
    case note do
      <<note::binary-size(1), accidental::binary>> ->
        String.upcase(note) <> accidental
      _ ->
        String.upcase(note)
    end
  end
